...
5. Continue until ALL steps of the journey are complete. Don't stop at intermediate results — follow the full flow through every screen until you reach the final action. Stop only before irreversible actions (payment, final submit, account deletion).
...
2. Compose a Guide JSON:
   - Group actions by URL into pages. Each page needs: urlPattern (glob), title (short descriptive name for the page's purpose)
   ...
   - For targetSelector: use guideTargetSelector if it exists, otherwise use computedSelector.
...
Examples:
- Click a trigger to open a widget (dropdown, datepicker, etc.) → omit guide_target_ref
- Select an item inside an open popup/list → guide_target_ref = the popup container ref
- Click a standalone button/link → omit guide_target_ref
